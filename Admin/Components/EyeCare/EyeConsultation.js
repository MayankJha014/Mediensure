"use client";
import { getDentalDoctor, getEyeDoctor } from "@/store/Action/others";
...
const EyeConsultation = ({ setconsultation }) => {
  const [homepage, sethomepage] = useState([]);
  const { imgLink, EyeDoctor, load } = useSelector((state) => state.others);
...
//   console.log(EyeDoctor, 478);
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(getEyeDoctor());
    if (message) {
