...
export const metadata = {
  ...
  import { call, getadmin, signinadmin } from "@/store/Action/Authentication";
  import { useRouter } from "next/navigation";
  import { RemoveError, clearError, clearmessage, isError } from "@/store/Reducer/AdminReducer";
import { toast } from "react-toastify";
  ...
  const Login = () => {
    ...
    const { message,error,isAuthenticated,loading } = useSelector((state) => state.adminReducer);
  ...
  }, [message,error,loading,isAuthenticated])
    ...
    return (
     <>
      ...
      <div className="side h-full flex-col  w-fit p-4 flex items-center justify-center max-lg:hidden ">
        <h1 className="text-5xl font-[poppins]">Hi , Welcome Back Admin 👋</h1>
        <img
          src={"/login.png"}
      ...
      <div className="h-full w-1/2 max-lg:w-full  relative flex flex-col items-center gap-6 justify-center">
        <h1 className=" text-4xl whitespace-nowrap font-bold font-[poppins]">
         Admin Please Login to MediEnsure
        </h1>
