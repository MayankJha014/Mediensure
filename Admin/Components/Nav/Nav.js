"use client";
import { getadmin, signoutadmin } from '@/store/Action/Authentication';
...
const Nav = () => {
    const { message,error,loading,isAuthenticated,admin } = useSelector((state) => state.adminReducer);
        ...
        <div className="flex items-center justify-start w-[30%] gap-5">
            ...
            >
              ...
              <li >
                ...
                {/* {isAuthenticated === false ? ""  : <Button onClick={()=>dispatch(signoutadmin())} variant='outlined'>LOGout</Button> } */}
            </div>
        </div>
    </div>
    </>
